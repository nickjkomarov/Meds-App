...
medsRA <- ra_data %>%
  rename("Composite.NNH" = "cNNH",
         ...
         "Name" = "Arm")
medsRA$cRDBen<-1/medsRA$Composite.NNT
medsRA<-medsRA %>%
  mutate(pc_rank_ben = rank(cRDBen)/length(cRDBen))
medsRA$fromNNT<-(medsRA$pc_rank_ben-0.5)*100
medsRA$cRDHarm<-1/medsRA$Composite.NNH
medsRA<-medsRA %>%
  mutate(pc_rank_harm = rank(cRDHarm)/length(cRDHarm))
medsRA$fromNNH<-(0.5-medsRA$pc_rank_harm)*100
medsRA$Name2<-sub("\\+.*", "", medsRA$Name)
...
# medsRA$Adjusted.Value.Score<-paste(medsRA$MeanLL,'-',medsRA$MeanUL)
