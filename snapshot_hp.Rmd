...
pb.val <- params$pb_val
nb.val <- params$nb_val
hp_data <- params$hp_data
```
...
### Efficacy measures
pb <- paste0("No reported bleeds (", pb.val, "% weight)")
nb <- paste0("Number of bleeding events per-person/per-month (", nb.val, "% weight)")
data <- c(pb, nb)
data <- data.frame(data) %>% rename('Efficacy Measures' = 'data')
